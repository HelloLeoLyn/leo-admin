...
import { key_dict } from '@/api/format'
...
export default {
  ...
  methods: {
    onWorkDialogConfirm(work) {
      work.content = JSON.stringify(this.taskProductDialog.tableData.map((o) => { return { 'id': o.id, type: 'product' } }))
    ...
    handWorkClick() {
      ...
      if (this.taskProductDialog.tableData.length + selection.length > 20) {
        this.$message.error('已经添加了' + this.taskProductDialog.tableData.length + '最多还能添加' + (20 - this.taskProductDialog.tableData.length) + '条数据')
        return
      }
